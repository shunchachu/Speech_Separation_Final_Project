#writefile model_SC_CHM_Fusion.py
...
class MS_SL2_split_model(nn.Module):
    ...
    def forward(self, x):
        ...
        for Slice in range(self.slice):
            ...
            # ✨ 核心修正：移至內迴圈外部，讓每個 Slice 都能動態計算各自的 w_scaled
            c = skip_connection.mean(dim=-1)
            alpha = th.sigmoid(self.fc(c))
            w_scaled = alpha * self.wList 
            
            # ✨ 極速廣播優化：替代 einsum 避免產生 4D 巨型暫存矩陣導致 OOM 
            w_sum = w_scaled.sum(dim=1).view(-1, 1, 1)
            slice_row_sum = skip_connection * w_sum
            
            # ✨ 通用化疊加邏輯：不論未來試驗改為幾個 Slice 都不會出錯
            if Slice == 0:
...
if __name__ == "__main__":
    ...
    x = nnet(x)
    print('測試成功！輸出 Speaker 數量：', len(x))